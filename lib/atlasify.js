...
class Atlasify {
    ...
    load(paths) {
        ...
        const output = new exporter_1.Exporter();
            ...
            this.packer.bins.forEach((bin, index) => {
                ...
                bin.rects.forEach(rect => {
                    ...
                    if (this.options.debug) {
                        const debugFrame = new jimp_1.default(sheet.frame.width, sheet.frame.height, this.debugColor);
                        image.blit(debugFrame, sheet.x + sheet.frame.x, sheet.y + sheet.frame.y);
                    }
                    image.blit(buffer, sheet.x, sheet.y, sheet.sourceFrame.x - sheet.frame.x, sheet.sourceFrame.y - sheet.frame.y, sheet.frame.width, sheet.frame.height);
                });
                image.write(binName, () => {
                ...
                fs_1.default.writeFileSync(`${basename}.json`, output.compile(view));
